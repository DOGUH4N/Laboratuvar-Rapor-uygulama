...
<thead>
	<tr>
		...
		<th><b>E-mail Adresi</b></th>
		<th><b>Randevu Tarihi</b></th>
		<th><b>Alınan Bölüm</b></th>
		<th><b>Ek Mesaj</b></th>
		<th><b>IP Adresi</b></th>
...
$sorgu= mysqli_query($baglanti,"select * from randevular");
while($satir=mysqli_fetch_array($sorgu))
{
	...
	echo '<td>'.$satir['randevutarih'].'</td>';
	echo '<td>'.$satir['bolum'].'</td>';
	echo '<td>'.$satir['mesaj'].'</td>';
	echo '<td>'.$satir['ipadresi'].'</td>';
	echo '<td>'.$satir['kayittarihi'].'</td>';
	echo '<td>'.$satir['durum'].'</td>';
	echo '<td> <a href="randevuguncellemeformu.php?id='.$satir['id'].'" >Güncelle🖌</a></td>';
	echo '<td> <a href="randevusil.php?id='.$satir['id'].'">Sil❌</a> </td>';
	echo '<tr>';	
}
